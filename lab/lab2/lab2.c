/* Lab 2 - Instructions
   
	1. Find and fix all the errors in this file.

	2. Leave a comment before each error that you fix explaining 
	   what the error was.

	3. Then copy the file into another file called  lab2.c  making 
	   sure to preserve errors.c (in case you need to refer to the 
		instructions again).

	4. Edit the header (these instructions) so that the proper header 
	   comment information is included in your file   lab2.c   which
		is the one that you will submit.
	
	5. After you have debugged and fixed the program, but before you 
	   submit your lab2.c file, test it with various numbers.  Use 2 
		(for triangle base) and 5 (for triangle height) as one of your 
		tests.  Notice the answer you get:
				Triangle area = (2 * triHeight) / 2 = 5 cm^2
		How would you change it so that the output says the following?
				Triangle area = (2 * 5) / 2 = 5 cm^2
		
	6. Do another test using 3 (for triangle base) and 5 (for triangle 
	   height) as one of your tests.  Notice the answer you get.  Leave 
		an additional comment at the bottom of your header comment showing 
		what answer you got when you used 3 and 5, and why that is. 

	7. After you have done all these steps, then you can submit your
	   fixed lab2.c to Vocareum.  Your TA can help you if you have 
		trouble doing that.  
*/



/* Program description:
	This program asks the user to enter two numbers: one for the base of 
	a triangle, one for the height.  It then calculates the area of the 
	triangle and shows the results to the user.

	The scanf() function is used for user input and also comes from stdio.h
	like printf().

	This is adapted from Participation Activity 1.6.2 in zyBooks.
*/

/* Header:
	Brayden Fidler
	CSPC 111 Lab Fall 2016
	Lab Section 5
	Program takes in the triangle base and height then calculates area.
	It also was an error correcting exercise

	The Results of Step Number 6:
	Triangle area = (3 * 5) / 2 = 7 cm^2

	It prints 7 because all of the variables including TriArea area are int variables which means the decimals
	(in this case the .5 on 7.5) are truncated.
*/
#include <stdio.h>

int main (void) {

	int triBase = 0;
	int triHeight = 0;
	int triArea = 0; //Changed the colon to a semicolon (syntax)

	// Get user input
	//Added a Semicolon to signal the end of the statement and a f to the print statement to correct it(syntax)
	printf("Enter triangle base (cm): "); 
	scanf("%d", &triBase); //Added a quotation mark to the right  of the d(syntax)
	
	printf("Enter triangle height (cm):" );
	scanf("%d", &triHeight); //Added & in front of triHeight(syntax)

	// Calculate triangle area
	/* I changed they type of comment from  a multi line comment to a single line comment
	 allowing the calculation to take place(logical)*/
	triArea = (triBase * triHeight) / 2;
	//Changed the colon to a semicolon to signal the end of the statement(syntax)

	// Print triangle base, height, area
	// Deleted Symbol for multiline comment above becuase it was unnecessary
	printf("Triangle area = (");
	printf("%d", triBase); //added a comma in between "%d" and triBase (syntax)
	printf(" * ");
	printf("%d", triHeight);//added in "%d", so the value of triHeight would print(syntax)
	printf(") / 2 = ");
	printf("%d", triArea);
	printf(" cm^2\n");

	return 0;
} //added a brace to complete the function(syntax)
